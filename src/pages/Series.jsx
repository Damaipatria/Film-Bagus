...
function Series() {
  ...
  useEffect(() => {
    document.title = 'Series | FILMBAGUS'
  }, [])
  ...
  return (
    <>
      <div className="dark:bg-slate-950">
        <div className="py-7">
          <h1 className="font-extrabold text-center text-4xl text-gray-900 dark:text-gray-200">Series</h1>
          <p className="lg:w-2/4 md:w-3/4 md:mt-1 mt-2 md:mx-auto mx-3 md:text-center text-justify text-gray-900 dark:text-gray-200">
            Tersedia banyak series berkualitas yang bisa anda nikmati bersama dengan keluarga dan teman  - teman anda, serta terdapat berbagai macam genre series yang dapat membuat anda bahagia, sedih, takut dan bahkan marah.
          </p>
        </div>
        <CarouselSeries />
        {movie.seriesGenre.slice(5, 10).map((item, index) => {
          return (
            <div key={index}>
              <CardSlider genre={item.id} name={item.name} type='series' />
            </div>
          )
        })}
...
function CarouselSeries() {
  ...
  return (
    <>
      <div className="flex justify-between items-center pt-5 lg:px-10 md:px-5 px-3 dark:bg-slate-950">
